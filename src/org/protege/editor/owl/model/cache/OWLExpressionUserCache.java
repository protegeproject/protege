...
public class OWLExpressionUserCache implements Disposable {
    ...
    private InternalFormEntityRenderer entityRenderer;
    ...
    private OWLExpressionUserCache(OWLModelManager mngr){
        ...
        entityRenderer = new InternalFormEntityRenderer();
        objectRenderer = new OWLObjectRendererImpl(mngr){
    ...
    public void add(OWLDescription owlDescription, String rendering) {
        ...
        String internalRendering = objectRenderer.render(owlDescription, null);
        ...
        if (!renderings.contains(internalRendering)){
            renderings.add(internalRendering);
        }
    }
    ...
    public List<String> getRenderings(OWLDescription descr) {
        ...
        for (String rendering : renderingsCache.get(descr)){
...
////////////////////////////////
    ...
    private String fromInternalForm(String rendererNeutralExpression) {
        String expression = rendererNeutralExpression;
        StringTokenizer tokenizer = new StringTokenizer(rendererNeutralExpression, WHITESPACE);
        while (tokenizer.hasMoreTokens()){
            String token = tokenizer.nextToken();

            OWLEntity entity = parseOWLEntity(token);
            if (entity != null){
                expression = expression.replaceAll(token, mngr.getRendering(entity));
            }
        }
        return expression;
    }
